    ...
    EnvironmentVariables=[
        ...
        {'Name': 'REPOSITORY_URI',
         'Value': Join("", [
             ...
             "/",
             "helloworld"])},
        {'Name': 'BROWSER_ENV_JSON', 'Value': 'JSON format required. ex) {"FOO":"foo", "BAR":"bar"}. DO NOT WRITE SECRET VALUES HERE. INSTEAD, USE NODE_ENV'},
        {'Name': 'NODE_ENV_JSON', 'Value': '{"BAZ":"baz", "HOO":"hoo"}'},
    ]
)
...
phases:
  pre_build:
    commands:
      ...
      - printf '{"tag":"%s", "node_env":%s}' "$(cat /tmp/tag.txt)" $NODE_ENV_JSON | tee /tmp/build.json
      - $(aws ecr get-login --no-include-email)
  build:
    commands:
      - echo $BROWSER_ENV_JSON
      - echo $BROWSER_ENV_JSON | jq -r "to_entries|map(\\"\\(.key)=\\(.value|tostring)\\")|.[]" | tr "\\n" " "
      - export BROWSER_ENV=$(echo $BROWSER_ENV_JSON | jq -r "to_entries|map(\\"\\(.key)=\\(.value|tostring)\\")|.[]" | tr "\\n" " ")
      - echo $BROWSER_ENV
